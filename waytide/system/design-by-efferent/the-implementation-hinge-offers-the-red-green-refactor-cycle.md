...
failure is local to the new test. Run the full suite after it. If a stable test failed, back
...
The implementation hinge is degenerate in that case. See the
...
- **Green** makes it pass.
- **Refactor is where the design is actually improved**, with the test holding the behavior
  still.
...
doubt, and whether the shape wants working out under a red. It is subtle, in that it cannot be read off the code,
...
exist to create. They compare real alternatives rather than ratifying a single proposal.

**How to apply:** at the implementation hinge for a unit's **first** implementation, offer the two
candidates through the selection UI. They are the working implementation, and the
red/green/refactor cycle. Where the cycle is chosen, write the red only if its cause reaches no stable test. Run
