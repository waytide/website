...
For a Ruby project it is settled by the `code/ruby` package's rule on running the suite. This rule fixes what the tree *is* and where it is derived from. Producing it is settled where a stack is.
