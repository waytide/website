...
separates these from an idea, an implementation record, or a migration record.

**A migration record is not one of these, and the distinction is the sharpest test of the rule.**
It was moved here on 2026-08-18 and moved back out the same day. It is one document rather than
entries that accrue, it is revised while the migration runs, and it is **prospective** — a
sequence of steps written to be executed. The three records above are retrospective accounts of
what happened. That a migration record is *kept* afterward makes it durable, not chronological,
and durability is not what this directory is for.
...
**Why:** the three were siblings of `ideas/`, `implementations/`, and `migration/` under
`waytide/local/`, which put a chronological account beside a statement of current state and
