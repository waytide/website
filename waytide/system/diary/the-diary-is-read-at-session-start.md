# The diary is read at the start of a session, with the rules

**Read the active writer's diary at the start of a session.** That is every entry in
`waytide/local/diary/<username>/`, where `<username>` is resolved by the
identity-resolution-contract rule. Read it after the rules and the vocabularies, and before the
first request. The diary is not a rule, and this is the one thing beside the rules and the
vocabularies that is read and does not bind. What it carries is what the writer thinks about
the work.

**Another writer's diary is not read.** A project may hold several, and each is one writer's
...
**Why:** the diary exists to carry the writer's thinking across sessions, and it carries
...
The cost is stated plainly in the a-diary-entry-is-read-and-never-followed rule. A writer
...
Reading one writer's diary alone follows from the same thing. An entry carries that writer's
