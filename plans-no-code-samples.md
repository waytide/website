...
Plans (`waytide/local/plans/*.md`), intentions (`waytide/local/intention/*.md`), and actions (`waytide/local/action/*.md`) must never contain code samples, and this rule governs the three identically. No code blocks, no literal method bodies, no "write this content" snippets. A plan describes the intended work and outcomes in prose. Code is produced later, interactively, when the user directs it.
