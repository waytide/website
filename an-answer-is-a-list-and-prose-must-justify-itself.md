...
An answer to the engineer is a **list**. Prose is not forbidden, and it is not the default. It
is written where one of the conditions below applies. Those conditions are the whole of what
justifies it.
...
for a two-clause answer organizes nothing. *Yes*, *The suite passes*, and *Correct — the rule says so
and I did not follow it* are answers. A list would only put marks in front of them.
...
its own answer. An agent's judgment about its own output is exactly what makes this rule
