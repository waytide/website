...
**`waytide/local/journal/<username>/` holds a writer's journal.** It is the one record in
`waytide/local/` the writer keeps. A project may hold the journals of more than one writer, and
each writer's entries sit under their own directory, named for their username. The
identity-resolution-contract rule resolves which writer is writing, and the journal-entries rule
states the directory at the entry. The decision log, the work session record, the loop record, and the experiment
record are all the agent's work, read by the writer. The journal runs the other way. The
...
rule name, every path, the `JOURNAL_WRITER` variable, and the `Journal:` commit prefix. Records
already written keep the word they were written with — the decision log, the implementation
records, and the writer's own entries are historical and are not revised. So `diary` still reads
in this repository's history, and it names nothing live.

**`waytide/local/journal/` holds the writers' directories and the writers file.** That file is
...
`YYYY-MM-DDTHH-MM-SS` form. The journal-entries rule states the condition and why the journal departs
...
and the artifacts are the project's. A journal entry is not. It is one writer's personal record,
...
who wrote it, which is what a journal kept by more than one writer needs.
...
**The journal is committed with the project.** It is not ignored and it is not kept outside the
...
16 2026)` — the words it read then, when this package was named `diary`. The prefix cost 13
characters and the date 17 more, against a subject line the `git` package's commit-message-widths
rule caps at 50. That left 20 characters for what the entry was about, and the date sat at the tail
where truncation takes it first — so it was carried at the subject's expense and no reader saw it
in a listing. The entry's filename holds the same date.

**`Journal:` rather than `Journal entry:`** for the same budget. It costs 9 characters where the
longer form costs 15, and it says the same thing to a reader scanning a log. The prefix was
`Diary:` for the few hours between the date being dropped and the package being renamed, both on
2026-08-18.
...
**The directory may not exist, and its absence is ordinary.** A project that keeps no journal has
no `waytide/local/journal/`, and nothing is wrong. So does a project whose other writers keep one
...
found without opening it. The local-time stamp is the one place the journal departs from that
convention, and the reason is above.

**How to apply:** resolve the active writer, and write an entry as a new file in
`waytide/local/journal/<username>/`. Give it the datetime prefix in the writer's local time, to the
