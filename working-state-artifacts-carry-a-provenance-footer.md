...
**The one exception — the decision log.** `waytide/local/log/` entries do **not** carry a footer. A log entry is deliberately one line: a `# <title>`, with no body and no template. See the decision-log-convention.
