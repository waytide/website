...
Use a named **`context`** to enclose a `test` when that outcome needs local instrumentation. That is an **explaining variable of its own**, or a
...
The outcome may be a **single assertion over values already in scope**, meaning the
shared controls and actuation at the top of the feature context. There, **name the
`test` directly and use no context**. A context that only holds a bare test adds a nesting level that carries no information. The name belongs on the test itself.
...
contexts for outcomes that actually use a local variable or add local
...
When flattening an over-nested test, flatten **only** where there is no code between the context and
