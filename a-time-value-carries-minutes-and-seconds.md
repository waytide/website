...
written to **seconds**. Write `3:45:12 PM PT`, not `3 PM PT` and not `3:45 PM PT`.
...
**A date alone is not a time.** A convention may record only a date, as a design's
...
the record. So this rule reaches **new values only**. Coarser values already
written stay as they are. That includes where a convention independently forbids
...
holds for a rule, a working-state artifact, a log entry, and any other file. When
